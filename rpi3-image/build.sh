#!/bin/sh
KERNEL_BUILD="../aarch64-kernel"
KERNEL="${KERNEL_BUILD}/linux-5.9.1"
UEFI_FIRMWARE="https://github.com/pftf/RPi3/releases/download/v1.30/RPi3_UEFI_Firmware_v1.30.zip"
BUILD_ROOT="../aarch64-root/rootfs"
ROOT="rootfs"
IMAGE="rpi3-gentoo.img"
...
{
[ -f $(basename $UEFI_FIRMWARE) ] || wget ${UEFI_FIRMWARE}
} &> /dev/null

echo "Installing UEFI firmware..."
{
unzip $(basename $UEFI_FIRMWARE) -d ${ROOT}/boot/efi
...
EOF
partprobe
truncate -s $(expr $(expr $(sfdisk -l ${IMAGE} -o end | tail -n 1) + 1) \* 512) ${IMAGE}
