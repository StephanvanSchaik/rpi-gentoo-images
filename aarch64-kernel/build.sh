...
echo "Building $(basename ${SOURCE_URL} .tar.gz) for AArch64..."
{
cd $(basename ${SOURCE_URL} .tar.gz)
make ARCH=arm64 CROSS_COMPILE=$HOME/x-tools/aarch64-unknown-linux-gnu/bin/aarch64-unknown-linux-gnu- defconfig
make ARCH=arm64 CROSS_COMPILE=$HOME/x-tools/aarch64-unknown-linux-gnu/bin/aarch64-unknown-linux-gnu- ${CPUS}
} &> /dev/null

echo "Installing $(basename ${SOURCE_URL} .tar.gz) for AArch64..."
{
mkdir ../boot
